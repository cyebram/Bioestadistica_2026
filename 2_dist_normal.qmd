...
subtitle: "Licenciatura en Ciencias Agrogenómicas"
lang: es
format:
  html:
    toc: true
...
El parámetro $\sigma$ controla la **dispersión** de la distribución, cuanto mayor es $\sigma$, más dispersos son los datos. A continuación se muestran las gráficas de la función de densidad de la distribución normal fijando el valor de la media $\mu = 2$ con diferentes valores de la desviación estándar.
...
ggplot(data.frame(x = x_inf, y = y_inf), aes(x = x, y = y)) +
  ...
  annotate("label", x = mu_inf, y = max(y_inf) * 1.1, 
           label = paste("μ =", mu_inf), color = "red", size = 5) +
  annotate("label", x = inflexion_x[1], y = inflexion_y[1] + 0.04,
           label = "μ - σ", color = "olivedrab", size = 5) +
  annotate("label", x = inflexion_x[2], y = inflexion_y[2] + 0.04,
           label = "μ + σ", color = "olivedrab", size = 5) +
...
ggplot(data.frame(z = z_seq, phi = phi_vals), aes(x = z, y = phi)) +
  ...
  geom_area(data = data.frame(z = z_3sigma, phi = phi_3sigma),
            aes(x = z, y = phi), fill = "yellow", alpha = 1) +
  geom_area(data = data.frame(z = z_2sigma, phi = phi_2sigma),
            aes(x = z, y = phi), fill = "orange", alpha = 1) +
  geom_area(data = data.frame(z = z_1sigma, phi = phi_1sigma),
            aes(x = z, y = phi), fill = "red", alpha = 1) +
  geom_vline(xintercept = c(-3, -2, -1, 0, 1, 2, 3), 
             linetype = "dashed", alpha = 0.6) +
  annotate("label", x = 0, y = 0.2, label = "68%", size = 4, fontface = "bold") +
  annotate("label", x = 1.5, y = 0.075, label = "13.5%", size = 3.5) +
  annotate("label", x = -1.5, y = 0.075, label = "13.5%", size = 3.5) +
  annotate("label", x = 2.5, y = 0.05, label = "2.35%", size = 3) +
  annotate("label", x = -2.5, y = 0.05, label = "2.35%", size = 3) +
...
Verificación numérica considerando $\mu=2$, $\sigma=1$ y $x=1.5$:
...
ggplot(df_cdf_efectos, aes(x = x, y = y, color = parametro)) +
  geom_line(linewidth = 1.2) +
  scale_color_manual(values = c("red", "green", "blue", "purple", "orange", "firebrick", "coral", "darkgreen")) + 
...
ggplot(df_comparacion_normal, aes(x = x, y = y, color = tipo)) +
...
ggplot(data.frame(x = x_q, y = cdf_q), aes(x = x, y = y)) +
  ...
  geom_point(data = df_cuantiles, aes(x = x, y = y), 
             color = "red", size = 3) +
  geom_text(data = df_cuantiles, aes(x = x-1.5, y = y, label = etiqueta),
            vjust = -0.5, hjust = 0.5, size = 4.5) +
...
rownames(tabla_principales) <- NULL
