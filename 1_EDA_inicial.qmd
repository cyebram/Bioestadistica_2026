...
color_species <- c("Adelie" = "#E69F00", "Chinstrap"= "#009E73", "Gentoo"= "#D55E00")
...
color_island <- c("Biscoe" = "#579D1C", "Dream"= "#0084D1", "Torgersen"= "#C5000B")
...
color_sex <- c("female" = "#4B1F6F", "male"= "#7AC36A" )
...
color_year <- c("2007" = "dodgerblue", "2008"= "darkslategray2", "2009"= "steelblue4")
...
Posteriormente exploraremos con más detalle las posibles relaciones entre variables categóricas, por ejemplo:
...
pie3D(data_species$Conteo, col= color_species, shade = 0.5, labels= data_species$species)
```
...
ggplot(datos)+
    geom_point(aes(x = flipper_length_mm, y = bill_depth_mm), color ="firebrick") +
    geom_smooth(aes(x = flipper_length_mm, y = bill_depth_mm), method = "lm") +
    #labs(x = val_x, y = val_y) +
    theme_bw()
```
...
for (val_y in num_var) {
  box_graf <- ggplot(datos)+
    ...
    scale_color_manual(values = color_island)+
    labs(y=val_y)+
    theme_bw()
...
Otra opción es realizar un *violin plot* con los cuantiles 0.25, 0.5 y 0.75, además de agregar los puntos de cada observación.
...
ggplot(datos)+
  geom_violin(aes(x=species, y=body_mass_g, fill=species), show.legend = FALSE,  quantiles = c(0.25, 0.5, 0.75), quantile.linetype = "solid", color="black", alpha = 0.5)+
